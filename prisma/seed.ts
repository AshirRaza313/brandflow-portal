...
async function seed() {
  ...
  const starter = await db.subscriptionPlan.create({
    data: {
      ...
      price: 0,
      annualPrice: 0,
      features: JSON.stringify(['50 Products', '100 Orders (Lifetime)', 'Basic Reports', 'Email Support', '3 Team Members']),
      teamLimit: 3,
      orderLimit: 100,
      productLimit: 50,
      trialDays: 14,
  ...
  const growth = await db.subscriptionPlan.create({
    data: {
      ...
      price: 4999,
      annualPrice: 49990,
      features: JSON.stringify(['200 Products', 'Unlimited Orders', 'Advanced Reports', 'Priority Support', '15 Team Members', 'Coupons & Discounts', 'Customer Loyalty', 'Marketing Tools', 'WhatsApp API', 'AI Tools', 'SEO Tools']),
      teamLimit: 15,
      orderLimit: -1,
      productLimit: 200,
      trialDays: 14,
