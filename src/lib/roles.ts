// ============================================================================
// 16-Role RBAC System for BrandFlow Portal
// ============================================================================
...
export const ROLES: RoleDefinition[] = [
  {
    ...
    description: "Full access to everything — the BrandFlow portal owner",
    level: 100,
...
export function canManageRoles(roleName: string): boolean {
  const managingRoles = ["platform_owner", "platform_admin", "brand_owner", "brand_admin"];
...
export function canAssignRole(
  ...
  const LEVELS: Record<string, number> = {
    ...
    owner: 90, admin: 80, ceo: 90, manager: 70, member: 20,
  };
  ...
  if (PLATFORM_ROLES.includes(target)) {
    if (!isPlatformOwner) {
      return { allowed: false, reason: "Platform roles can only be assigned by the BrandFlow owner.", code: "PLATFORM_ROLE_BLOCKED" };
    }
    return { allowed: true };
  }
...
/** Check if a role is a platform-level role (includes legacy 'owner' for first user) */
export function isPlatformRole(roleName: string): boolean {
  return roleName === "platform_owner" || roleName === "platform_admin" || roleName === "owner";
}
...
/** Check if a role is a client (business owner) */
export function isClientRole(roleName: string): boolean {
  return roleName === "brand_owner" || roleName === "brand_admin";
}
...
/** Check if a role is a brand-level owner/admin */
...
export function getAccessibleSections(roleName: string): string[] {
  ...
  if (role.permissions.all) {
    ...
    return [
      ...
      "campaigns", "seo-manager", "social-media", "email-marketing", "ad-manager", "loyalty", "seasonal-sales", "influencers", "affiliates", "coupons", "marketing-calendar",
      "returns", "sla-engine", "support-tickets", "packaging", "shipping", "suppliers", "warehouse", "product-reviews", "team-management", "team-chat", "support-chat", "attendance", "payroll",
  ...
  const permissionToSection: Record<string, string[]> = {
    ...
    marketing: ["campaigns", "seo-manager", "social-media", "email-marketing", "ad-manager", "loyalty", "seasonal-sales", "influencers", "affiliates", "coupons", "marketing-calendar"],
    operations: ["returns", "sla-engine", "support-tickets", "packaging", "shipping", "suppliers", "warehouse", "product-reviews", "team-management", "team-chat", "support-chat", "attendance", "payroll"],
...
export function isSectionAccessible(sectionId: string, roleName: string): boolean {
  ...
  // Subscriptions: visible to platform roles and brand owners/admins
