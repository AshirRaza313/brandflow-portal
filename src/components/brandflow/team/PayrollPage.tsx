...
export function PayrollPage() {
  ...
  const fetchExpenses = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/expenses?orgId=${organization.id}`);
      if (!res.ok) {
    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Button
            variant="outline" size="sm"
            onClick={() => toast.success("Export completed successfully!")}
            className={cn(isGold && "border-amber-500/20 text-amber-400 hover:bg-amber-500/10", isDark && !isGold && "border-white/10 text-slate-300 hover:bg-white/5")}
          >
          ...
          <motion.div key="reports" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
            <Card className={cardClass}>
              <CardContent className="p-8 text-center">
                <TrendingUp className={cn("h-12 w-12 mx-auto mb-3", isDark ? "text-slate-500" : "text-muted-foreground")} />
                ...
                <div className="flex flex-wrap gap-3 justify-center">
                  ...
                  <Button variant="outline" onClick={() => toast.success("Excel report exported successfully!")} className={cn(isGold && "border-white/10 text-slate-300 hover:bg-white/5")}>
                    Export Excel
                  </Button>
                </div>
              </CardContent>
