...
export function TeamPage() {
  const { organization, appTheme, user } = useBrandFlowStore();
  ...
  const fetchMembers = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/team?orgId=${organization.id}`);
  ...
  const handleRoleUpdate = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/organization/members/${editMember.id}/role`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  const handleRemove = async () => {
    ...
    setRemoving(true);
    try {
      const res = await fetch(`/api/team?memberId=${removeMember.id}`, { method: "DELETE" });
      if (!res.ok) {
