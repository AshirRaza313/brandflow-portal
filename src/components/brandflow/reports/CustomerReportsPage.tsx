...
import { Download, Users, UserPlus, Heart, Wallet, Loader2, BarChart3, FileText, FileSpreadsheet, ChevronDown } from "lucide-react";
...
export function CustomerReportsPage() {
  ...
  const handleExport = async () => {
    ...
    setExporting(true);
    try {
      const res = await fetchWithAuth(`/api/reports/export?type=customers&orgId=${organization.id}`);
      if (res.ok) {
